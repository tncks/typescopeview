...
{ int i;
  INDENT;
  while (tree != NULL) {
    printSpaces(tree->nodekind == ExpK && tree->kind.exp == ConstK && tree->isArray == -1); // nested switch case (two dimension)
      ...
      case DeclK:
        switch (tree->kind.decl) {
          case FunK:
            ...
            break;
          case VarK:
            fprintf(listing, "Variable Declaration: name = %s, type = %s", tree->attr.name, (tree->type == Integer) ? "int" : "void");
            // if(tree->type == Integer) fprintf(listing, "int");
            // else fprintf(listing, "void");
          ...
          case ParamK:
            ...
            fprintf(listing, "Parameter: name = %s, type = %s", tree->attr.name, (tree->type == Integer) ? "int" : "void");
            // if(tree->type == Integer) fprintf(listing, "int");
            // else fprintf(listing, "void");
            if (tree->isArray) fprintf(listing, "[]");
            fprintf(listing, "\n");
